(function (global, factory) {
  ...
  function voronoiMapSimulation(data, arrayOfNodes, handleWeightOption) {
	arrayOfFixedPositionsNodes =Array.from(arrayOfNodes); 
    //begin: constants
	...
	if(handleWeightOption!=null)//Ala add this to let choose between handleOverweighted
		handleOverweightedVariant=handleWeightOption;
    ...
    function adapt(polygons, flickeringMitigationRatio) {
      var adaptedMapPoints;
       if(handleWeightOption!=null)
         adaptPositions(polygons, flickeringMitigationRatio);
	   else 
		   adaptPositionsAsInPaper(polygons);
      ...
      if(handleWeightOption!=null)
	    ...
	    adaptWeightsAsInPaper(polygons);
    ...
    function adaptPositions(polygons, flickeringMitigationRatio) {		
      ...
      for (var i = 0; i < siteCount; i++) {
		...
		if(polygon==null || polygon==undefined){
			      //newMapPoints.push(null);
				  continue;
		}
		//if(i==parentIndexStartDragChild){
	    if(arrayOfFixedPositionsNodes.indexOf(i+"")!=-1){
		  //console.log("adapt: arrayOfFixedPositionsNodes");
		  //console.log(arrayOfFixedPositionsNodes);
		  
          //console.log("adaptPositions i="+i);
		...
		//console.log(polygon );
		//console.log("i="+i);
		//console.log(polygon.site.originalObject);
        mapPoint = polygon.site.originalObject;
      ...
      do {
        ...
        for (var i = 0; i < siteCount; i++) {
          ...
          for (var j = i + 1; j < siteCount; j++) {
            tpj = mapPoints[j];
			if(tpj=="undefined"  || tpi=="undefined" || tpj==null  || tpi==null ){
				continue;
			}
    ...
    function handleOverweighted1(mapPoints) {
      ...
      do {
        ...
        for (var i = 0; i < siteCount; i++) {
		    //if(i==parentIndexStartDragChild){
			if(arrayOfFixedPositionsNodes.indexOf(i+"")!=-1){	
              //console.log("handleOverweighted1 mouse parent i="+i);
		      continue;
		    }	
		
          tpi = mapPoints[i];
          for (var j = i + 1; j < siteCount; j++) {
		    //if(j==parentIndexStartDragChild){
			/*if(arrayOfFixedPositionsNodes.indexOf(j+"")!=-1){	
              //console.log("handleOverweighted1 mouse parent j="+i);
		      continue;
		    }*/	
...
function adaptPositionsAsInPaper(polygons){   
      //console.log("adaptPositions");		
       
      var  polygon, s, centroid, dx, dy;
      ...
      for (var i = 0; i < siteCount; i++) {
		...
		//we  change this line to match orginal paper Algorithm2 :Update steps
		var dBorder = distanceBorder(s, polygon);
		//console.log("distanceBorder="+dBorder);
        var sqrtWs=  Math.sqrt(s.weight); // s.weight;//
		//console.log("sqrtWs="+sqrtWs);
		var minSqrtWsDistanceBorder=Math.min(sqrtWs,dBorder);
		s.weight=  minSqrtWsDistanceBorder ** 2;   
    ...
    function adaptWeightsAsInPaper(polygons){

      var  polygon,  currentArea, adaptRatio, adaptedWeight;
	  var S_array = [];
	  ...
	  for (var i = 0; i < siteCount; i++) {	
		 ...
		 if(polygon==null || polygon==undefined){
		  ...
		  var s = polygon.site.originalObject;
		  S_array.push(s);
	  }
      ...
      for (var i = 0; i < siteCount; i++) {			
			...
			currentArea = d3Polygon.polygonArea(polygon);
			adaptRatio = s.targetedArea / currentArea;


			//console.log("adapt weight ws-sqrt");
			//console.log(s);
			//console.log(s.weight+" "+Math.sqrt(s.weight)); 
			var WNew = Math.sqrt(s.weight) * adaptRatio;
			//console.log(WNew);
			var filteredS_array=S_array.filter((a) => a!=null && a.index != s.index) ;// .data.originalData.id
			//console.log(filteredS_array);
		    var NNs=filteredS_array. reduce((a, b) => distance(a, s) < distance(b, s)  ? a : b);
		    //console.log(NNs);
			var WMax=distance(NNs, s) ;
			//console.log(WMax);
		    adaptedWeight= Math.min(WNew,WMax)** 2;
		    adaptedWeight = Math.max(adaptedWeight, epsilon);
		    s.weight = adaptedWeight;
      }
      
	}

        function distanceBorder(tpi, polygon) {
 
	       var distanceBorder=Number.MAX_VALUE;
		   //console.log(tpi);
	       //tpi = mapPoints[i];
		   var xP=tpi.x;// tpi[0]; //
		   var yP=tpi.y; // tpi[1]; //
		   ...
		   var edgeIndex = 0;
           var edgeVertex0 = polygon[polygon.length - 1];
           var edgeVertex1 = polygon[edgeIndex];
		   
		   while (edgeIndex < polygon.length) {
			   var xa = edgeVertex0[0];
               var ya = edgeVertex0[1];
			   var xb = edgeVertex1[0];
			   var yb = edgeVertex1[1];   
			   
			   var proj=footvec(xa,ya,xb,yb,xP,yP);
			   //console.log("proj="+proj);
			   var xptmp=proj[0];
			   var yptmp=proj[1];
			   			   
			   var dproj=distvec(xptmp,yptmp,xP,yP);			   
			   if (dproj<distanceBorder) {
				 distanceBorder=dproj;				 
			   }
			   
				edgeIndex++;
				edgeVertex0 = edgeVertex1;
				edgeVertex1 = polygon[edgeIndex];			   
		   }	
